...
<head>
    ...
    <link href="https://fonts.bunny.net/css?family=inter:300,400,500,600,700,800,900|sora:400,500,600,700,800&display=swap" rel="stylesheet" />
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    @stack('styles')
</head>
<body class="bg-surface-50 text-gray-800 font-sans antialiased" x-data="{ mobileMenu: false, searchOpen: false }">

    {{-- Announcement Bar --}}
    <div class="relative bg-gradient-to-r from-brand-950 via-brand-900 to-brand-950 text-white/90 text-xs font-medium tracking-wide overflow-hidden">
        <div class="absolute inset-0 opacity-30" style="background-image:radial-gradient(circle at 20% 50%, rgba(16,185,129,0.4), transparent 40%), radial-gradient(circle at 80% 50%, rgba(20,184,166,0.3), transparent 40%);"></div>
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 flex justify-between items-center">
            <div class="flex items-center gap-2">
                <span class="relative flex w-2 h-2">
                    <span class="absolute inline-flex w-full h-full rounded-full bg-brand-400 opacity-75 animate-ping"></span>
                    <span class="relative inline-flex w-2 h-2 rounded-full bg-brand-400"></span>
                </span>
                <span class="tracking-wider">Serving All of Lebanon</span>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16 md:h-18">
                ...
                <a href="{{ url('/') }}" class="flex items-center shrink-0 group">
                    <img src="{{ asset('images/logo.jpeg') }}" alt="Unidrug Lebanon" class="h-14 md:h-16 w-auto transition-transform duration-300 group-hover:scale-105">
                </a>
                ...
                <nav class="hidden md:flex items-center gap-1">
                    ...
                    <a href="{{ route('contact') }}" class="btn-primary inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-full">
    ...
    {{-- Footer --}}
    <footer class="relative overflow-hidden mt-16 md:mt-24">
        {{-- Wave separator with brand accent --}}
        <div class="relative text-surface-950">
            <svg viewBox="0 0 1440 80" class="w-full h-14 md:h-20 block" preserveAspectRatio="none" fill="currentColor">
                <path d="M0,80 L0,30 Q360,0 720,30 T1440,30 L1440,80 Z"/>
            </svg>
            <div class="absolute inset-x-0 bottom-0 h-px" style="background:linear-gradient(to right, transparent, rgba(16,185,129,0.6), transparent);"></div>
        </div>
        <div class="bg-surface-950 relative">
            <div class="absolute inset-0 opacity-40 pointer-events-none" style="background-image:radial-gradient(circle at 10% 20%, rgba(16,185,129,0.08), transparent 50%), radial-gradient(circle at 90% 80%, rgba(20,184,166,0.06), transparent 50%);"></div>
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 pt-4">
                <div class="grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-12">
                    {{-- Brand --}}
                    <div class="md:col-span-5">
                        <div class="flex items-center gap-3 mb-5">
                            <img src="{{ asset('images/logo.jpeg') }}" alt="Unidrug Lebanon" class="h-14 w-auto brightness-0 invert opacity-90">
                        </div>
                        ...
                        <div class="flex gap-3 mt-6">
                            @if(!empty($siteSettings['facebook_url'] ?? ''))
                                <a href="{{ $siteSettings['facebook_url'] }}" target="_blank" class="w-10 h-10 bg-white/5 hover:bg-brand-600 ring-1 ring-white/10 hover:ring-brand-500 rounded-xl flex items-center justify-center text-gray-400 hover:text-white transition-all hover:-translate-y-0.5">
                            ...
                            @if(!empty($siteSettings['instagram_url'] ?? ''))
                                <a href="{{ $siteSettings['instagram_url'] }}" target="_blank" class="w-10 h-10 bg-white/5 hover:bg-brand-600 ring-1 ring-white/10 hover:ring-brand-500 rounded-xl flex items-center justify-center text-gray-400 hover:text-white transition-all hover:-translate-y-0.5">
                            ...
                            @if(!empty($siteSettings['tiktok_url'] ?? ''))
                                <a href="{{ $siteSettings['tiktok_url'] }}" target="_blank" class="w-10 h-10 bg-white/5 hover:bg-brand-600 ring-1 ring-white/10 hover:ring-brand-500 rounded-xl flex items-center justify-center text-gray-400 hover:text-white transition-all hover:-translate-y-0.5">
                            ...
                            @if(!empty($siteSettings['linkedin_url'] ?? ''))
                                <a href="{{ $siteSettings['linkedin_url'] }}" target="_blank" class="w-10 h-10 bg-white/5 hover:bg-brand-600 ring-1 ring-white/10 hover:ring-brand-500 rounded-xl flex items-center justify-center text-gray-400 hover:text-white transition-all hover:-translate-y-0.5">
                            ...
                            @if(!empty($siteSettings['youtube_url'] ?? ''))
                                <a href="{{ $siteSettings['youtube_url'] }}" target="_blank" class="w-10 h-10 bg-white/5 hover:bg-brand-600 ring-1 ring-white/10 hover:ring-brand-500 rounded-xl flex items-center justify-center text-gray-400 hover:text-white transition-all hover:-translate-y-0.5">
                            ...
                            @if(!empty($siteSettings['twitter_url'] ?? ''))
                                <a href="{{ $siteSettings['twitter_url'] }}" target="_blank" class="w-10 h-10 bg-white/5 hover:bg-brand-600 ring-1 ring-white/10 hover:ring-brand-500 rounded-xl flex items-center justify-center text-gray-400 hover:text-white transition-all hover:-translate-y-0.5">
                            ...
                            @endif
                            <a href="mailto:{{ $siteSettings['email'] ?? 'info@unidrug.com' }}" class="w-10 h-10 bg-white/5 hover:bg-brand-600 ring-1 ring-white/10 hover:ring-brand-500 rounded-xl flex items-center justify-center text-gray-400 hover:text-white transition-all hover:-translate-y-0.5">
            ...
            <div class="border-t border-white/5 relative">
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-col sm:flex-row justify-between items-center gap-3">
                    <p class="text-xs text-gray-500 tracking-wide">&copy; {{ date('Y') }} <span class="text-gray-300 font-medium">Unidrug Lebanon</span>. All rights reserved.</p>
                    <div class="flex items-center gap-5 text-xs text-gray-500">
                        ...
                        <span class="hidden sm:inline-flex items-center gap-1.5 text-gray-600">
